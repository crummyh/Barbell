...
def test_user(session):
    return user_crud.create(
        session, UserCreate(username="fixture", email="fixture@test.com", password="pw")
    )
...
def test_batch(session, test_user):
    return upload_batch_crud.create(
        session,
        UploadBatchCreate(
            capture_time=datetime.now(), file_size=1111, user_id=test_user.id
        ),
    )
...
def test_crud_cycle(
    session: Session, model, create_schema, crud_module, delete, kwargs, test_batch
):
    if model is Image:
